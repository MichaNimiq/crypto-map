...
<template>
  <DropdownMenu.Root>
    <DropdownMenu.Trigger v-bind="$attrs">
      <IconThreeDots
        class="w-5 h-5 transition-colors" :class="{
    ...
    <DropdownMenu.Portal>
      ...
      >
        ...
        <DropdownMenu.Item
          v-if="copyIsSupported" class="flex px-4 py-2 text-white transition-colors outline-none cursor-pointer select-none hover:text-white/80"
          @click="copy(url())"
        >
        ...
        <DropdownMenu.Item
          as="a" :href="`/location/report?uuid=${location.uuid}`"
