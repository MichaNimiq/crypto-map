...
<template>
  <div
    ...
    <div class="grid items-center grid-cols-[auto_1fr_auto] grid-rows-2 grid-flow-dense gap-x-2">
      ...
      <span v-if="cryptocity.locationsCount > 1" class="text-sm leading-[1] text-space/70">{{ $tc('{count} locations',
        cryptocity.locationsCount) }}</span>
      <button type="button"
    ...
    <p v-for="(p, i) in cryptocity.description" :key="i" class="text-sm text-space/80 text-pretty"
      :class="i === 0 ? 'pt-3' : 'pt-2'">
      {{ p }}
    </p>
    <Button :href="cryptocity.url" class="mt-2 !p-0 !h-auto [&_span]:!text-sm" bg-color="transparent" size="sm"
      text-color="sky">
      <template #label>
        {{ cryptocity.url }}
      </template>
