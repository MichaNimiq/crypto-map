...
function onCryptocityClick(cryptocityName: Cryptocity) {
  console.log(cryptocityName, cryptocities.value.data)
  ...
  <CustomMarker v-for="c in clusters" :key="c.id"
    ...
    <OnClickOutside @trigger="isMobile && toggleClusterExpansion(c, $event)">
      <ul :data-cluster-id="c.id" relative :style="getClusterStyles(c)" @pointerover="e => onPointer(e, c)"
        ...
        <li relative z-10>
          <button centered text="14 neutral-0 dark:white/80" bg="neutral dark:blue hover:neutral-900"
            ring="1.5 neutral/10" transition-colors font-bold rounded-full shadow cursor-pointer aspect-square
        ...
        transform: translateX(calc(var(--offset-1) * (1 - var(--expanded)) + var(--offset-2) * var(--expanded))) rotate(calc((1 - var(--expanded)) * -90deg));
          ...
          `">
          <div centered p-4 bg-neutral-0 rounded-full shadow cursor-pointer aspect-square
