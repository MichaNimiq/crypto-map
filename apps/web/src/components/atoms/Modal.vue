...
<template>
  <DialogRoot v-model:open="open" @update:open="$event ? $emit('open') : $emit('close')">
    ...
    <DialogPortal>
      ...
      <DialogContent class="fixed max-desktop:bottom-0 desktop:top-1/2 desktop:left-1/2 max-h-[85dvh] max-w-lg desktop:-translate-x-1/2 desktop:-translate-y-1/2 rounded-md z-20 overflow-y-auto data-[state=open]:animate-fade md:rounded-lg md:max-w-lg focus:outline-none w-full py-8 text-left  bg-white border border-[#e9e9ed] rounded-t-lg shadow-lg">
