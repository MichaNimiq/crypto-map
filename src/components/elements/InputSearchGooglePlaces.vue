...
const placeholder = t(props.placeholder || "placeholders.search-places")

let autocomplete: google.maps.places.SearchBox
const autocompleteElement = ref<HTMLInputElement>()
...
onMounted(async () => {
	...
	autocomplete = new google.maps.places.SearchBox(autocompleteElement.value)
	autocomplete.addListener("places_changed", onPlacesChanged)
})
...
function onPlacesChanged() {
	...
	if (placeData.place.photos)
		placeData.imageUrl = placeData.place ? placeData.place.photos[0].getUrl() : null
	...
	if (placeData.place.types.length > 0) {
		for (const key in placeData.place.types) {
			if (Object.prototype.hasOwnProperty.call(placeData.place.types, key)) {
				...
				if (type) placeData.locationTypes.add(type)
			}
		}
	}
}
...
<template>
	<div class="input-wrap">
		...
		<div class="input-icon-wrap">
			...
			/>
			<IconSvg class="magnifier" iconIndex="icon-search" />
		</div>
		<div v-if="placeData.place !== null" class="places-resultset">
			<div class="name" v-html="placeData.place.name"></div>
			<div class="address" v-html="placeData.place.adr_address"></div>
			<div class="rating-location">
				<div v-if="placeData.place.rating > 0" class="stars-wrap">
					<IconSvg
				...
				<div v-if="placeData.locationTypes.size > 0" class="location-types">
					<span v-for="locationType in placeData.locationTypes" :key="locationType">{{
						locationType
					}}</span>
				</div>
			</div>
			<div v-if="placeData.imageUrl !== ''" class="image-wrap">
